...
class DeleteExpenseCategoryStateMachine(BaseStateMachine):
    def __init__(self, client: BaseClient, repository: BaseRepository):
        self._category_service = ExpenseCategoryActionsService(client, repository)
        self._action_for_state: Dict[State, Callable[[Event], None]] = {
            State.START: self._category_service.start_processing,
            State.RECEIVED_CATEGORY: self._category_service.delete_expense_categories,
        }
